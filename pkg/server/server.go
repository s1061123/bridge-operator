...
import (
	...
	bridgeconfclientscheme "github.com/s1061123/bridge-operator/pkg/client/clientset/versioned/scheme"
...
func (s *Server) bridgeInfoUpdate() {
	...
	bridgeInfoList, err := s.bridgeConfClient.OpenShiftIOV1alpha1().BridgeInformations().List(context.TODO(), metav1.ListOptions{})
	if err != nil {
		...
		for _, bridgeInfo := range bridgeInfoList.Items {
			if bridgeInfo.Status.Node == s.Hostname {
				...
				if !ok {
					err := s.bridgeConfClient.OpenShiftIOV1alpha1().BridgeInformations().Delete(context.TODO(), bridgeInfo.Name, metav1.DeleteOptions{})
					if err != nil {
	...
	for bridgeName := range s.managedBridges {
		...
		bridgeInfo, err := s.bridgeConfClient.OpenShiftIOV1alpha1().BridgeInformations().Get(context.TODO(), infoName, metav1.GetOptions{})
		if err != nil {
			if !apierrors.IsNotFound(err) {
				klog.Errorf("bridge info update: get error: %v", err)
			}
			bridgeInfo = newBridgeInfo(infoName)
			bridgeInfo, err = s.bridgeConfClient.OpenShiftIOV1alpha1().BridgeInformations().Create(context.TODO(), bridgeInfo, metav1.CreateOptions{})
			if err != nil {
		...
		newBridgeInfo, err = s.bridgeConfClient.OpenShiftIOV1alpha1().BridgeInformations().Update(context.TODO(), newBridgeInfo, metav1.UpdateOptions{})
		if err != nil {
